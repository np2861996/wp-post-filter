...
<div class="wppf-post-grid  wppf-medium-<?php echo esc_html($postgrid); ?> wppf-columns <?php echo esc_html($css_class); ?>">
	<div class="wppf-post-grid-content <?php if ( !has_post_thumbnail() ) { echo esc_html('no-thumb-image'); } ?>">
		...
		if ( has_post_thumbnail() ) { ?>
			<div class="wppf-post-image-bg" style="<?php echo esc_html($height_css); ?>">
				<a href="<?php echo esc_link($post_link); ?>">
					<img src="<?php echo esc_link($post_featured_image); ?>" alt="<?php the_title(); ?>" />
				</a>
			</div>
		<?php } ?>
		<div class="wppf-content-above-image">
			<h2 class="wppf-post-title">
				<a href="<?php echo esc_link($post_link); ?>"><?php the_title(); ?></a>
			</h2>
			...
			if($showDate == "true" || $showAuthor == 'true' || $show_comments =="true") { ?>
				<div class="wppf-post-date">
					<?php if($showAuthor == 'true') { ?>
						<span class="wppf-user-img"><img src="<?php echo esc_html(WPPF_URL); ?>assets/images/user.svg" alt=""> <?php the_author(); ?></span>
					<?php } ?>
					...
					<?php if(!empty($comments) && $show_comments == 'true') { ?>
						<span class="wppf-post-comments">
							<img src="<?php echo esc_link(WPPF_URL); ?>assets/images/comment-bubble.svg" alt="" />
			...
			if($showContent == "true") { ?>
				<div class="wppf-post-content">
						...
						<?php if($show_read_more == 'true') { ?>
							<a href="<?php echo esc_link($post_link); ?>" class="readmorebtn"><?php _e( 'Read More', 'wp-post-filter' ); ?></a>
						<?php } ?>
				</div>
			<?php } ?>
		</div>
